...
//acAmpTX
int acAmpDataOut[5];
int acAmpDataDefault[5] = {0x104, 0x180, 0x180, 0x180, 0x0FC}; //Default data for the A\C amplifier TX; basicly it means I am here and no buttons have been pressed.
...
void setup() {
  ...
  Serial.begin (115200, SERIAL_8N1, true); // 9 bit mode
  Serial2.begin (4800, SERIAL_8N1, true);// 9 bit mode
...
{
  if (acAmpRxData[0] == 0x00F) // we are off apart from showing Mode settings
  {
    acAmpRunning = false;
  }

  acAmpAmbient = (acAmpRxData[0] == 0x10E) ? true : false; // 10D Normal || 10E Ambient
...
// Get data from the acAmplifier - it's 9N1 and inverted. Note we don't sit and wait around for a full payload.
void acAmpGetSerialData()
{
  ...
  { //acAmpOn = true;
    ...
    if (acAmpNewByte == 0x00F || acAmpNewByte == 0x10D || acAmpNewByte == 0x10E) // We don't know the last byte but we know the first byte will be one of these
    {
      acAmpIndex = 0;
...
void checkFanRotation()
{
  ...
  if  (aFanNew != aFanOld || bFanNew != bFanOld)
  {
    if ((aFanNew && !aFanOld && !bFanNew && !bFanOld) || (!aFanNew && aFanOld && bFanNew && bFanOld) )  //Right movement
    {
      if (!confMode)
      {
        acAmpDataOut[3] = 0x090; acAmpDataOut[4] = 0x1EC;
      }
      else
      {
    ...
    else if ((aFanNew && aFanOld && !bFanNew && bFanOld) || (!aFanNew && !aFanOld && bFanNew && !bFanOld)) //Left movement
    {
      if (!confMode)
      {
        acAmpDataOut[3] = 0x0F0; acAmpDataOut[4] = 0x18C;
      }
      else
      {
...
void checkTempRotation()
{
  ...
  if  (aTempNew != aTempOld || bTempNew != bTempOld)
  {
    if ((aTempNew && !aTempOld && !bTempNew && !bTempOld) || (!aTempNew && aTempOld && bTempNew && bTempOld) )  //Right movement
    {
      if (!confMode)
      {
        acAmpDataOut[3] = 0x081; acAmpDataOut[4] = 0x1FB;
      }
      else
      {
    ...
    else if ((aTempNew && aTempOld && !bTempNew && bTempOld) || (!aTempNew && !aTempOld && bTempNew && !bTempOld)) //Left movement
    {
      if (!confMode)
      {
        acAmpDataOut[3] = 0x087; acAmpDataOut[4] = 0x0F5;
      }
      else
      {
...
void shortButtonAction(byte _shortPushButton)
{
  if (!confMode) // Lookup the correct transmission data if normally running
  {
    ...
    {
      case 1: acAmpDataOut[1] = 0x082; acAmpDataOut[4] = 0x0FA;
        break;
      case 2: acAmpDataOut[1] = 0x090; acAmpDataOut[4] = 0x1EC;
        break;
      case 3: acAmpDataOut[1] = 0x084; acAmpDataOut[4] = 0x1F8;
        break;
      case 4: acAmpDataOut[1] = 0x0A0; acAmpDataOut[4] = 0x1DC;
        break;
      case 5: acAmpDataOut[1] = 0x0C0; acAmpDataOut[4] = 0x1BC;
        break;
      case 6: acAmpDataOut[1] = 0x088; acAmpDataOut[4] = 0x1F4;
        break;
      case 7://we should never get here with RX8;
        break;
      case 8: acAmpDataOut[1] = 0x081; acAmpDataOut[4] = 0x1FB;
        break;
    }
  }
}
...
void longButtonAction(byte _longPushButton)
{
  ...
  if (!confMode) // we can set control options on long button press
  {
    switch (_longPushButton)
    {
      ...
      case 7 :
        break;
      case 8 : acAmpAmbient = !acAmpAmbient ? true : false; acAmpDataOut[2] = 0x0A0; acAmpDataOut[4] = 0x1DC;
        break;
    }
  }
...
void acAmpSend() //Send hardware changes to the AC Amplifier
{
  ...
  {
    Serial2.write9bit(acAmpDataOut[acAmpCycle]);
  }
